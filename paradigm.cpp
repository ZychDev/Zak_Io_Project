...
class paradigm_graph
{
  ...
  std::string aft1_loop = "		</MasterView></Include></ModelChildren></ModelRelationshipContainer></ModelChildren></ModelRelationshipContainer>";
  ...
  public:
    ...
    paradigm_graph()
    {
      ...
      std::ofstream ofile("paradig.xml",std::ios_base::app);
      if (!ifile.is_open()) 
      { 
...
{
ofile<<"<Include  From=\""<<f2[i].first<<"\" Id=\""<<f2[i].first<<"_"<<f2[i].second<<"\" Name=\"1\" To=\""<<f2[i].second<<"\">"<<std::endl<<"<Stereotypes>"<<std::endl<<"<Stereotype Idref=\"inc\" Name=\"Include\">"<<std::endl<<"</Stereotypes>"<<std::endl<<"<MasterView>"<<std::endl<<"<Include Idref=\"c"<<i<<"\" Name=\"1\"/>"<<std::endl<<"</MasterView>"<<std::endl;
}
...
ofile<<aft3_loop<<std::endl;
    ...
    for(auto i = f4.begin() ; i != f4.end() ; ++i)
    {
        ...
        for(auto j = i->second.begin() ; j != i->second.end() ; ++j)
        {
          ...
          ofile<<"<Include Background=\"rgb(122, 207, 245)\" Foreground=\"rgb(0, 0, 0)\" From=\""<<i->first<<"_s"<<"\" Id=\""<<"n"<<n<<"\" Model=\""<<i->first<<"_"<<j->first<<"\" To=\""<<j->first<<"_s\">"<<std::endl<<"<ElementFont Color=\"rgb(0, 0, 0)\" Name=\"Dialog\" Size=\"11\" Style=\"0\"/>"<<std::endl<<"<Caption Height=\"30\" Visible=\"true\" Width=\"82\" X=\"\" Y=\"\"/>"<<std::endl<<"</Include>"<<std::endl;
